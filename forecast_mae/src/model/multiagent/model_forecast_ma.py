...
class ModelForecastMultiAgent(nn.Module):
    def __init__(
        ...
        # register_buffer: 注册一个持久性缓冲区. 缓冲区: 在模型中存储, 不需要梯度更新的固定数据. 
        # 持久性: 缓冲区会随着模型一起保存和加载. 
        # torch.eye(self.num_modes): 创建一个大小为 num_modes times num_modes 的单位矩阵（对角线为 1，其余为 0），通常用于生成 one-hot 编码。
        # 
    ...
    def forward(self, data):
        ...
        #  为每个模式（trajectory mode）提供一个独特的标识符。将 one-hot 编码添加到 x_actors 和 cls_token，帮助模型区分不同的轨迹模式。
