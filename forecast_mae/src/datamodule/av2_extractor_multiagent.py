...
class Av2ExtractorMultiAgent:
    ...
    def process(self, raw_path: str, agent_id: str = None):
        ...
        scored_actor = list(filtered_track_id.unique())
        focal_agent_id = df["focal_track_id"].values[0]
        print("scored_actor:", scored_actor)
        print("focal_ actor: ", focal_agent_id)

        # filtered_track_ids = [df["track_id"][i] for i, m in enumerate(mask) if m.item()].unique()
