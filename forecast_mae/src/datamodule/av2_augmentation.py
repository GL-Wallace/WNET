...
class Av2Augmentation:
    ...
    def save(self, file: Path):
        ...
        for single in data:
            for agent_id, values in single.items():
                # print(f"Agent ID: {agent_id}")
                save_file = self.save_path / (file.stem +"_"+ agent_id +".pt")
                torch.save(values, save_file)

    def get_data(self, file: Path):
        data_list = []
        df, am, scenario_id = load_av2_df(file)
        object_category = torch.from_numpy(df["object_category"].values).int()
        mask = object_category == 2
        filtered_track_id = df.loc[mask.numpy(), "track_id"]
        scored_actors = list(filtered_track_id.unique())
        for agent_id in ["AV", df["focal_track_id"].values[0]] + scored_actors:
            local_df = df[df["track_id"] == agent_id].iloc
            data_list.append(self.process(df, am, scenario_id, agent_id, local_df))
        return data_list

    def process(self, df, am, scenario_id, agent_id, local_df):
        ...
        actor_ids = list(cur_df["track_id"].unique()) # 获取所有的actors列表
        ...
        scored_agents_mask = object_category > 1.5 # keep scored tacks
