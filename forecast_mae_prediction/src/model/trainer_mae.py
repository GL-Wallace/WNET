...
from forecast_mae_prediction.src.utils.optim  import WarmupCosLR
...
class Trainer(pl.LightningModule):
    ...
    def training_step(self, data, batch_idx):
        out = self(data)
        self.log("train_loss", out["loss"], on_epoch=False, on_step=True)
        return out["loss"]

    def validation_step(self, data, batch_idx):
        out = self(data)
        self.log("val_loss", out["loss"], on_epoch=True, on_step=False)
        self.log("val_hist_loss",  out["hist_loss"], on_epoch=True, on_step=False)
        self.log("val_fut_loss",  out["future_loss"], on_epoch=True, on_step=False)
        self.log("val_lane_loss",  out["lane_pred_loss"], on_epoch=True, on_step=False)
