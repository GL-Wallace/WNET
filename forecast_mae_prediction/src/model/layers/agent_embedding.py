...
class AgentEmbeddingLayer(nn.Module):
    ...
    def forward(self, x):
        """x: [B, C, T]"""
        x = self.embed(x) # C[4-32] -> [16, 50, 32]
        ...
        for i in range(len(out) - 1, 0, -1):
            ...
            scale_factor = round(int(laterals[i - 1].shape[-1])/ int(laterals[i].shape[-1]), 1)
...
class ConvTokenizer(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x) # Conv1d(4, 32, k=3, s=1, p=1) [16, 4, 50] -> [16, 32, 50]
        x = x.permute(0, 2, 1)  # B, C, L -> B, L, C [16, 50, 32]
